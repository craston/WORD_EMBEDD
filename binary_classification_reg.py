...
def main(_):
  ...
  '''
  batch_xs = np.load("Embeddings/{0}_train_data_{1}.npy".format(args.dataset, args.split))
  batch_ys = np.load("Embeddings/{0}_train_labels_{1}.npy".format(args.dataset, args.split))
  test_xs = np.load("Embeddings/{0}_test_data_{1}.npy".format(args.dataset, args.split))
  test_ys = np.load("Embeddings/{0}_test_labels_{1}.npy".format(args.dataset, args.split))
  '''
  # Train
  for _ in range(1000):
